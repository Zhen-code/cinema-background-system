...
import {reqCategorys, reqAddFilm, reqUpdateFilm} from "../../api";
...
class UpdateFilm extends React.Component{
  ...
  validateFields(async (errors, values) => {
    if(!errors){
      ...
      const detail=this.getdetail.current.gethtml();
            ...
            const result=await reqUpdateFilm({_id,parentId,name,type,detail,imgs});
            if(result.status===0){
  ...
  componentWillMount(){
      this.setState((pre,now)=>{
          pre._id = this.props.location.state._id;
          pre.movieInfo = this.props.location.state
      });
